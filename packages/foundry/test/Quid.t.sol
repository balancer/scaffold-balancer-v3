...
import { Quid } from "../contracts/Quid.sol";
contract QuidTest is BaseVaultTest {
    ...
    function setUp() public virtual override {
        ...
        nftRouter = new Quid(IVault(address(vault)), weth, address(dai), permit2);
    ...
    function testRemoveWithNonOwner() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Quid.WithdrawalByNonOwner.selector, lp, bob, nftTokenId)
        );
        vm.prank(lp);
        nftRouter.removeLiquidityProportional(nftTokenId, minAmountsOut, false);
    }
    ...
    function testAddFromExternalRouter() public {
        ...
        uint256[] memory maxAmountsIn = [dai.balanceOf(bob), usdc.balanceOf(bob)].toMemoryArray();
        vm.expectRevert(abi.encodeWithSelector(Quid.CannotUseExternalRouter.selector, router));
        vm.prank(bob);
