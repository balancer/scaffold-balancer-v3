...
contract NFTLiquidityStakingHook is INFTLiquidityStakingHook,IHooks, BaseHooks, ERC721, Ownable, VaultGuard, ReentrancyGuard ,IVotes{
...
function setGovernor(address _governor) external onlyOwner {
    governor = NFTGovernor(payable(_governor));
     ...
     function stakingInfo(address user, address pool) external view override returns (
    ...
    return (info.stakedAmount, info.stakingStartTime, info.lastMilestoneTime, info.currentTier, info.lastRewardClaim);
}
...
function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
    ...
    _totalSupplyCheckpoints[_currentCheckpoint] = _totalSupplyCheckpoints[_currentCheckpoint - 1] + newPower - _votePowerCheckpoints[_currentCheckpoint - 1][account];
}
...
function _checkAndMintNFT(address user, address pool) internal {
 ...
 function onAfterRemoveLiquidity(
   ...
   function upgradeNFT(uint256 tokenId) external nonReentrant {
    ...
    require(block.timestamp >= lastUnstakeTime[msg.sender][_tokenIdToPool[tokenId]] + UPGRADE_COOLDOWN_PERIOD, "Upgrade cooldown period not over");

    address pool = _tokenIdToPool[tokenId];
    StakingInfo storage user_stakingInfoMap = _stakingInfoMap[msg.sender][pool];

    uint256 newTier = _calculateTier(user_stakingInfoMap.stakedAmount, block.timestamp - user_stakingInfoMap.stakingStartTime);
