...
import {ConstantProductFactory} from "../src/factories/ConstantProductFactory.sol";
 ...
 */
contract DeployConstantProductPool is PoolHelpers, Script {
    function deployConstantProductPool(
        address token1,
        address token2
        ...
        CustomPoolConfig memory poolConfig = getProductPoolConfig(
            token1,
            token2
        );
        InitializationConfig memory initConfig = getProductPoolInitConfig(
            token1,
            token2
        );
        ...
        // Deploy a factory
        ConstantProductFactory factory = new ConstantProductFactory(
            IVault(helperConfig.getConfig().vault),
            365 days
        ); //pauseWindowDuration
        console.log(
            "Constant Product Factory deployed at: %s",
            address(factory)
        );
