...
/**
 * @title Deploy Exponential Product Pool
 * @notice Deploys, registers, and initializes a Exponential Product Pool
 ...
 */
contract DeployExponentialProductPool is PoolHelpers, ScaffoldHelpers {
    function run() external virtual {
        ...
        address factory = DevOpsTools.get_most_recent_deployment(
            "ExponentialProductFactory", // Must match the mock token contract name
            block.chainid
        );
        ...
        address pool = ExponentialProductFactory(factory).create(
            regConfig.name,
            regConfig.symbol,
            regConfig.salt,
        ...
        console.log("Exponential Product Pool deployed at: %s", pool);
        ...
        console.log("Exponential Product Pool initialized successfully!");
        vm.stopBroadcast();
    }
    ...
    function getRegistrationConfig(
        ...
        string memory name = "Exponential Product Pool"; // name for the pool
        string memory symbol = "EPP"; // symbol for the BPT
    ...
    function getInitializationConfig(
        ...
        uint256 minBptAmountOut = 1e18; // Minimum amount of pool tokens to be received
